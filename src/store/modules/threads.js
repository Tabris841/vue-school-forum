...
export default {
  ...
  actions: {
    async createThread({ commit, state, rootState }, { text, title, forumId }) {
    ...
    updateThread({ state, commit, rootState }, { title, text, id }) {
      return new Promise(async resolve => {
        ...
        const updates = {
          [`posts/${thread.firstPostId}/text`]: text,
          [`posts/${thread.firstPostId}/edited`]: edited,
    ...
    fetchThread: ({ dispatch }, { id }) =>
      dispatch('fetchItem', { resource: 'threads', id, emoji: '📄' }, { root: true }),
